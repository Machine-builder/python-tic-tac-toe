...
class SERVER: ...
class CLIENT: ...
class ServerClientSystemCLIENT: ...
...
class SOCKETBASE():
    ...
    def hsend_e(self, event = None, sending_socket = None, headersize = 16):
        '''uses a smart header system, and sends a python object, using pickle'''
        pickled = UTIL.dump_pickle_object( event.data )
        return self.headersend( pickled, sending_socket, headersize )
...
class ServerClientSystem():
    ...
    def main(self):
        ...
        for notified_socket in read_sockets:
            if notified_socket == self.server.conn:
                ...
                actual_message = ServerClientSystemMessage( message, notified_socket )
    ...
    def validate_for_send(self, obj):
        if isinstance(obj, BasicEvent): obj = obj.data
        return obj
    
    def send_to_conn(self, conn, obj):
        '''sends a python object to a conn'''
        obj = self.validate_for_send(obj)
        data = UTIL.dump_pickle_object( obj )
        header = self.server.generate_header( data )
        self.send_msg_to_client( conn, header+data )
    ...
    def send_to_clients(self, obj):
        ...
        data = UTIL.dump_pickle_object( obj )
        header = self.server.generate_header( data )
        byte_message = header+data
        for client_socket in self.clients:
            self.send_msg_to_client( client_socket, byte_message )

class ServerClientSystemMessage():
    '''A ServerClientSystem message,
    has attributes is_pickled, header & data.
    data is the actual object of the message.'''
    def __init__(self, data, from_conn=False):
        self.header = None
        if type(data)==dict:
            if 'data' in data:
...
class BasicEvent(object):
    """A basic event,
    used to generate message objects"""
    def __init__(self, event_data, **kwargs) -> None:
        self.from_conn = False
        if isinstance(event_data, str):
            self.data = {'event': event_data}
            self.data.update(kwargs)
        elif isinstance(event_data, ServerClientSystemMessage):
            self.data = event_data.data
            self.from_conn = event_data.from_conn
        else:
    ...
    def __repr__(self):
        return f"BasicEvent<{self.event}>"
